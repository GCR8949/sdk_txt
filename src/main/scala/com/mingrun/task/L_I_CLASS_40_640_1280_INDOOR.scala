...
object L_I_CLASS_40_640_1280_INDOOR {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      ...
      *
      * i_day_tradtkpi_N_RDD : cdate + province + city + operator + xyID_N + overage_ratio + sinr3_ratio
      */
...
//    executeBatchByRDD(i_tradtkpiRDD, sql_L_TRADTKPI_40_640_1280_INDOOR, 10)
    ...
    val i_day_tradtkpi_1280RDD = getI_Day_TradtKpi_N_RDD(i_tradtkpiRDD, 1280)
    val i_day_tradtkpi_640RDD = getI_Day_TradtKpi_N_RDD(i_tradtkpiRDD, 640)
    val i_day_tradtkpi_40RDD = getI_Day_TradtKpi_N_RDD(i_tradtkpiRDD, 40)
...
//    executeBatchByRDD(i_day_tradtkpi_640RDD, sql_L_TRADTKPI_640_DAY_INDOOR, 7)
    ...
    //  因为40栅格的数据入库时，不入栅格编号，所以单独写了个入库函数
    executeBatchByTRADTKPI40(i_day_tradtkpi_40RDD)

//    i_day_tradtkpi_1280RDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\kpi_1280_1")
//    i_day_tradtkpi_640RDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\kpi_640_1")
//    i_day_tradtkpi_40RDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\kpi_40_1")
...
//    proresRDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\prores1")
    ...
    /**
      ...
      *
      * i_day_N_RDD : date + province + city + operator + xyID_N + whole_ratio + build_ratio + optimization_ratio + not_four_g_ratio + not_network_ratio
      *
    ...
    val i_day_1280RDD = getI_Day_N_RDD(proresRDD, 1280)
    val i_day_640RDD = getI_Day_N_RDD(proresRDD, 640)
    val i_day_40RDD = getI_Day_N_RDD(proresRDD, 40)

//    i_day_1280RDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\day_1280_1")
//    i_day_640RDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\day_640_1")
//    i_day_40RDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\day_40_1")

    //  L_I_CLASS_GROUP_DAY_INDOOR
    val i_groupRDD = getI_GroupRDD(proresRDD, gridMapBroadcast)

//    i_groupRDD.coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\group1")
...
//    sc.parallelize(accum.value.toList).map(x => x._1 + "," + x._2).coalesce(1).saveAsTextFile("G:\\TwoWF\\　　\\conf1")
  ...
  def getI_TradtKpiRDD(dataByXYIDRDD: RDD[(String, scala.Iterable[String])]): RDD[String] = {
    dataByXYIDRDD.mapValues(lines => {
      ...
      list.foreach(line => {
        ...
        val rsrp = items(2).toLong
        val sinr = items(3).toLong
  ...
  /**
    ...
    * @return 对应栅格大小的数据
    */
  def getI_Day_TradtKpi_N_RDD(i_tradtkpiRDD: RDD[String], gridSize: Int): RDD[String] = {
    ...
    i_tradtkpiRDD.map(line => {
      ...
      val key = date + "," + province + "," + city + "," + operator + "," + xyID_N
  ...
  /**
    ...
    * @return 对应栅格大小的数据
    */
  def getI_Day_N_RDD(proresRDD: RDD[String], gridSize: Int): RDD[String] = {
    val n = gridSize match {
      case 1280 => 4
      case 640 => 5
      case 40 => 6
    ...
    proresRDD.map(line => {
      ...
      datas.foreach(data => {
        ...
        if ((G - B1 - B2) / totalSmpl > 0.85) i = i + 1
        if (B2 / totalSmpl > 0.85) j = j + 1
        if (B1 / totalSmpl > 0.85) k = k + 1
      })
  ...
  /**
    ...
    * @return 各个场景下的数据统计结果
    */
  def getI_GroupRDD(proresRDD: RDD[String], gridMapBroadcast: Broadcast[scala.collection.Map[String, String]]): RDD[String] = {
    proresRDD.map(line => {
      val items = line.split(",", -1)
  ...
  def executeBatchByTRADTKPI40(i_day_tradtkpi_40RDD: RDD[String]): Unit = {
    i_day_tradtkpi_40RDD.foreachPartition(lines => {
      var connection: Connection = null
      var pstmt: PreparedStatement = null
      val sqlStr = "insert into L_TRADTKPI_40_DAY_INDOOR (TIME_GRANULARITY, PROVINCE, CITY, OPERATOR, COVERAGE_RATIO, SINR3_RATIO) values(?,?,?,?,?,?)"
